...
echo "$($PSQL "SELECT GREATEST(winner_goals, opponent_goals) FROM games ORDER BY greatest DESC LIMIT 1")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COUNT(winner_goals) FROM games WHERE winner_goals>'2'")"

echo -e "\nWinner of the 2018 tournament team name:"
echo "$($PSQL "SELECT name FROM teams LEFT JOIN games on teams.team_id = games.winner_id LIMIT 1;")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games AS winner ON teams.team_id = winner.winner_id FULL JOIN games AS opponent ON teams.team_id = opponent.opponent_id WHERE winner.game_id>=24 OR opponent.game_id>=24 ORDER BY name")"
